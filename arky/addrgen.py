...
def _chr(v): return chr(v).encode() if __PY3__ else chr(v)
...
def base58_encode_padded(s):
    ...
    res = base58_encode(int('0x' + (s.hex() if __PY3__ else s.encode('hex')), 16))
    pad = 0
    for c in s:
        if c == chr(0):
            pad += 1
